...
for(i in 1:length(dtlist)){
  parsing_xls_pred(x = dtlist[i], ix = i, m = char_mon[mm])
}
...
parsing_xls_sifat = function(x){
  # x = dtlist[3]
  wb <- loadWorkbook(file = paste0("input/2019/",x,"/Sifat/sifat hujan.2019.xlsx") )
  iyang = which(wb$sheet_names == "SEP")
  writeData(wb, sheet = iyang,startCol = 3, startRow = 3, x = substr(as.character(as.matrix(All$Sifat2)),1,1) )
  saveWorkbook(file = paste0("input/2019/",x,"/Sifat/sifat hujan.2019.xlsx"), overwrite = T, wb = wb)
  # saveWorkbook(file = paste0("input/2019/",x,"/Sifat/sifat hujan.2019.xls"), overwrite = T, wb = wb)
  
  wb2 <- loadWorkbook(file = paste0("input/2019/",x,"/Sifat/sifat hujan.new2019.xlsx") )
  iyang2 = which(wb2$sheet_names == "SEP")
  writeData(wb2, sheet = iyang2,startCol = 5, startRow = 3, x = as.numeric(as.matrix(All$Sifat)) )
  saveWorkbook(file = paste0("input/2019/",x,"/Sifat/sifat hujan.new2019.xlsx"), overwrite = T, wb = wb2)
  # saveWorkbook(file = paste0("input/2019/",x,"/Sifat/sifat hujan.new2019.xls"), overwrite = T, wb = wb2)
}

for(i in 1:length(dtlist)){
  parsing_xls_sifat(dtlist[i])
}